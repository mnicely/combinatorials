...
int main( int arg, char **argv ) {
    ...
    for ( int d = numDevices; d > 0; d-- ) {
        ...
        gpuWork[d - 1].treesPerDevice = temp - ( d - 1 ) * chunk;
        temp -= chunk;
        if ( chunk < temp )
            temp = chunk;
    }
...
#pragma omp parallel
    {
        ...
        checkCudaErrors( cudaMalloc( reinterpret_cast<void **>( &comboStruct[ompId].d_treeCombos ),
                                     gpuWork[ompId].treesPerDevice * k_numEdges * sizeof( unsigned int ) ) );
        checkCudaErrors( cudaMalloc( reinterpret_cast<void **>( &comboStruct[ompId].d_treeScores ),
...
#pragma omp parallel
    {
        ...
        checkCudaErrors( cudaMemcpyAsync( &h_holdTrees[gpuWork[ompId].offset * k_numEdges],
                                          comboStruct[ompId].d_treeCombos,
                                          gpuWork[ompId].treesPerDevice * k_numEdges * sizeof( unsigned int ),
                                          cudaMemcpyDeviceToHost ) );

        checkCudaErrors( cudaMemcpyAsync( &h_holdScore[gpuWork[ompId].offset],
                                          comboStruct[ompId].d_treeScores,
                                          gpuWork[ompId].treesPerDevice * sizeof( unsigned int ),
                                          cudaMemcpyDeviceToHost ) );
...
#pragma omp parallel
    {
        ...
        checkCudaErrors( cudaFree( comboStruct[ompId].d_treeCombos ) );
        checkCudaErrors( cudaFree( comboStruct[ompId].d_treeScores ) );
